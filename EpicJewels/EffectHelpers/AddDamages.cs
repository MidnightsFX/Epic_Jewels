...
using Jewelcrafting;
using UnityEngine;
using EJLogger = EpicJewels.Common.EJLogger;
            ...
            if (hit.GetAttacker() is Player)
            {
                ...
                if (player.GetEffectPower<GemEffects.EitrFused.Config>("Eitr Fused").Power > 0 || player.GetEffectPower<GemEffects.Spellsword.Config>("Spellsword").Power > 0)
                {
                    ...
                    if (player.HaveEitr(eitr_cost))
                    {
                        ...
                        // Logger.LogDebug($"Eitr powered attack powered up multiplier {eitr_fused_powerup} cost {eitr_cost}");
                        ...
                        EJLogger.LogDebug($"Eitr powered attack not triggered due to cost {eitr_cost}");
                    }
                }
