            ...
            internal static void Prefix(HitData hit)
            {
                if (hit.GetAttacker() is Player attacker)
                {
                    ...
                    if (player != null && player.GetEffectPower<Config>("Coin Hoarder").Power > 0) {
                        float damage_multiplier = CoinHoarderBonusCalc(player);
                        // Logger.LogDebug($"Coinhorder bonus multipler {damage_multiplier} coin log bonus: {coin_bonus} inv coins: {inv_coins}");
